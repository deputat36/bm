...
Создать воспроизводимый процесс редакционной и юридической приёмки семи материалов без автоматического присвоения статусов `passed` и без снятия `noindex`.
...
Для шести юридически значимых материалов проверяются десять критериев:
...
редакционно принято: 0/7
юридически принято: 0/6
index_ready: 0/7
noindex: сохраняется
```
