...
## 8. Пятая очередь: статьи справочника

| Старый URL | Целевой URL | Действие | Содержание |
|---|---|---|---|
| `/spravochnik/proverka-dokumentov-novostroyki/` | `/guides/proverka-dokumentov-novostroyki/` | retain_content | migrated |
| `/spravochnik/proektnaya-deklaratsiya/` | `/guides/proektnaya-deklaratsiya/` | retain_content | migrated |
| `/spravochnik/kak-vybrat-planirovku/` | `/guides/kak-vybrat-planirovku/` | retain_content | migrated |

Старые статьи заменены нейтральными переходными страницами. Из них удалены старый брендинг, архивная навигация, ссылки на объектные черновики и прежние CTA.

Новые версии статей расширены и зарегистрированы как активные страницы портала. Они остаются `noindex,follow` до редакционной и юридической проверки.

Аудит:

```text
docs/portal/LEGACY_GUIDE_MIGRATION_2026-07-13.md
```

## 9. Что переносится до редиректов
...
HTML-карта проверяется отдельно:

```bash
npm run validate:html-sitemap
```

## 12. Условия выпуска редиректа
...
3. Провести контролируемый dry-run форм на рабочем домене.
4. После отдельного согласования провести одну реальную проверку доставки заявки.
5. Провести редакционную и юридическую проверку новых статей справочника.
6. Определить платформу размещения и синтаксис серверных редиректов.
7. После проверки помечать отдельные маршруты как `redirect_ready`.
