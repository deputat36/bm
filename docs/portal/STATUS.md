...
- ипотечные CTA автоматически сохраняют `residential_complex_id` в query-параметре `object`;
- ипотечная форма автоматически выбирает объект и синхронизирует `residential_complex_id`;
- ипотечная страница показывает безопасное подтверждение выбранного объекта без обещаний цены, наличия или одобрения;
...
добавление query-параметра object в ипотечные ссылки
автоподстановку объекта и уведомление на ипотечной странице
...
Переход с карточки объекта:

```text
mortgage_open
→ /ipoteka/?object=<residential_complex_id>#lead
→ автоподстановка residential_complex
→ синхронизация residential_complex_id
→ сообщение о выбранном объекте
...
Произвольное значение query string не выводится пользователю. Название берётся только из внутреннего списка допустимых объектов.
...
Текущий единый сценарий карточек:

```text
первый экран
→ статус информации
→ ответы покупателю
→ короткая заявка или ипотечный расчёт
→ сохранение объекта
→ страница благодарности
```

Автоматический валидатор контролирует формы, идентификаторы, разделение статусов, FAQ, CTA, ипотечный контекст и адаптивные CSS-селекторы.
...
2. Проверить переходы по трём ссылкам `?object=<id>#lead` и сохранение `residential_complex_id` в тестовой заявке.
3. После согласования выполнить одну реальную тестовую заявку на `/contacts/`.
4. Сохранить точные ссылки на карточку ЕИСЖС, декларацию и разрешение Просторной 4А.
5. Сверить critical claims по первичным источникам.
6. Проверить происхождение и права на визуализации.
7. Подготовить фактические рекламные ссылки и QR-коды из UTM-реестра.
8. После начала трафика сравнивать `lead_form_view` → `lead_form_start` → `lead_submit` по короткой, подробной и ипотечной форме.
9. Отдельно измерять переходы из CTA с `placement=project_faq` и `action=mortgage_open`.
10. Собрать источники по другим новостройкам Борисоглебска.
11. Определить платформу размещения и правила серверных редиректов.
