...
- на первом экране добавлена короткая форма `homepage_quick_selection`, а подробная анкета сохранена ниже для квалифицированных обращений;
- мобильная панель действий теперь ведёт к короткой форме; сохранены ARIA-статусы, контроль фокуса и мобильные настройки клавиатуры;
...
7. После начала трафика сравнивать `lead_form_view` → `lead_form_start` → `lead_submit` по форме и размещению CTA.
