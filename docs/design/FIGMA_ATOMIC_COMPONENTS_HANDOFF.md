...
Light используется на белых и нейтральных поверхностях. Hero используется на тёмном первом экране и CTA-секциях.
...
Mobile использует системную ширину 336 px. Ответ не обещает цену, наличие, ипотеку или юридический результат без проверки.
...
Quick содержит 3 поля, Detailed — 8. Оба варианта содержат обязательное согласие перед CTA и реальные Form Field/Button instances. Submit action является exposed instance.
...
Link Card является полностью кликабельной карточкой-направлением без отдельного CTA. Destination хранится на экземпляре через shared plugin data `portal-v2 / route`.
...
Footer использует Brand `Context=Dark`, `surface/emphasis`, `text/inverse` и `coral/100`.
...
Footer сохраняет формулировку, что портал не является официальным сайтом застройщика и уточняет сведения по публичным источникам.

Подробности: `docs/design/FIGMA_SITE_FOOTER_HANDOFF.md`.

## Итог

Подготовлено 14 ComponentSet и 119 вариантов:
...
- shared plugin data для route и form metadata.
...
- exposed instance в Content Card и Lead Form Card;
- `Show step label` в Step Card;
- `portal-v2 / route` для Link Card;
- Footer phone-route и sources-route;
- полный набор из 119 вариантов.
...
2. сделать screenshots всех четырнадцати ComponentSet;
...
14. проверить Site Footer на ширинах 1200 и 336 px;
15. проверить phone-route, sources-route, privacy-route и contacts-route;
