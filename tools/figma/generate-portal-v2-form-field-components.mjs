...
for (const controlType of ["Input", "Select", "Textarea"]) {
  for (const state of ["Default", "Focus", "Disabled"]) {
    const node = component("Control=" + controlType + ", State=" + state, "VERTICAL");
    node.itemSpacing = 8;
    node.counterAxisSizingMode = "FIXED";
    node.resize(520, controlType === "Textarea" ? 180 : 136);
    ...
    control.resize(520, controlType === "Textarea" ? 104 : 52);
    ...
    const value = await text(control, defaultValue, {
      ...
      width: controlType === "Select" ? 430 : 472,
    ...
    node.description = "Form Field · " + controlType + " · " + state + ". Минимальная интерактивная высота 48 px, обязательный заметный focus state.";
    variants.push(node);
  }
}
const set = combine(variants, stage, "Form Field");
set.description = "Portal v2 Form Field · Control × State";
...
await text(notes, "Label не заменяется placeholder. Focus должен быть заметен без изменения размера элемента. Disabled применяется только при понятной причине. Согласие на обработку данных остаётся отдельным обязательным элементом формы.", {
  name: "Notes body",
  styleName: "Typography/Body",
  width: 1080,
...
process.stdout.write(buildComponentRuntime({
  ...
  title: "Form Field",
  description: "Input, Select и Textarea в состояниях Default, Focus и Disabled для коротких и подробных форм портала.",
