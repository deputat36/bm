...
const PROJECTS = [
  {
    ...
    optionCount: 1,
    formIds: ["catalog_prostornaya_4a_quick_consultation", "catalog_prostornaya_4a_priority_lead"]
  },
  {
    ...
    optionCount: 2,
    formIds: ["catalog_aerodromnaya_18g_quick_consultation", "catalog_aerodromnaya_18g_priority_lead"]
  },
  {
    ...
    optionCount: 2,
    formIds: ["catalog_sennaya_76_quick_consultation", "catalog_sennaya_76_priority_lead"]
  }
];
...
for (const forbidden of [
  "localStorage", "sessionStorage", "URLSearchParams", "window.location.search", "history.pushState",
...
for (const project of PROJECTS) {
  ...
  if (count(html, `data-track-placement="${project.placement}"`) !== 1) errors.push(`${project.page}: expected exactly one intent CTA placement`);
  if (count(html, `<option>${project.interest}</option>`) !== project.optionCount) errors.push(`${project.page}: expected ${project.optionCount} matching interest options`);
  if (count(html, "form[data-lead-form]") > 0) errors.push(`${project.page}: literal selector text unexpectedly present in HTML`);
  if (count(html, "<form ") !== 2) errors.push(`${project.page}: active form count must remain 2`);
  for (const formId of project.formIds) {
    if (count(html, `data-form-id="${formId}"`) !== 1) errors.push(`${project.page}: form id must remain unique: ${formId}`);
  }
