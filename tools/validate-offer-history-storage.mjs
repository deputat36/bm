...
const expectedCanonicalFields = new Set([
  "event_id", "event_type", "observed_at", "object_id", "section_or_entrance",
  "apartment_number_public", "source_id", "value", "previous_event_hash", "event_hash"
]);
exactSet(canonicalFields, expectedCanonicalFields, `${HISTORY_PATH}: canonical history fields`);

const columns = Array.isArray(storage.columns) ? storage.columns : [];
const columnNames = new Set(columns.map((column) => column.name));
const expectedPhysicalColumns = new Set([
  "event_id", "offer_identity", "object_id", "section_or_entrance", "apartment_number_public",
  "event_type", "observed_at", "price", "availability_status", "source_id", "source_checked_at",
  "previous_event_hash", "event_hash", "idempotency_key", "created_at"
]);
exactSet(columnNames, expectedPhysicalColumns, `${STORAGE_PATH}: physical columns`);
...
const mapping = storage.canonical_mapping || {};
exactSet(new Set(Object.keys(mapping)), canonicalFields, `${STORAGE_PATH}: canonical_mapping keys`);
for (const [canonicalField, physicalFields] of Object.entries(mapping)) {
  if (!Array.isArray(physicalFields) || physicalFields.length === 0) {
    errors.push(`${STORAGE_PATH}: canonical_mapping.${canonicalField} must map to at least one physical column`);
    continue;
  }
  for (const physicalField of physicalFields) {
    if (!columnNames.has(physicalField)) errors.push(`${STORAGE_PATH}: canonical_mapping.${canonicalField} references unknown column ${physicalField}`);
  }
}
exactSet(new Set(mapping.value || []), new Set(["price", "availability_status"]), `${STORAGE_PATH}: canonical value mapping`);
for (const field of [...canonicalFields].filter((item) => item !== "value")) {
  exactSet(new Set(mapping[field] || []), new Set([field]), `${STORAGE_PATH}: canonical_mapping.${field}`);
}
...
if (storage.hash_chain?.first_event_previous_hash !== null || history.hash_chain?.first_event_previous_hash !== null) errors.push(`${STORAGE_PATH}: first event previous hash must remain null`);
...
for (const key of ["name", "phone", "email", "client_fixation_id", "seller_internal_id", "page_url", "referrer", "user_agent"]) {
  ...
  if (columnNames.has(key)) errors.push(`${STORAGE_PATH}: forbidden column present: ${key}`);
}
